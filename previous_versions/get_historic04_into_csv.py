...
# receives a list of tickers and outputs time series with date and close
    ...
    for indx, ticker in enumerate(ticker_list):
        ...
        if indx == 0:
            ...
            # if length of series is < trade_days, add 0s at end
            if length < trade_days:
                # iterate over list...
                for index, each in enumerate(range(length + 1)):
                    # split each element into list
                    splitter = splitdata[index].split(',')
                    # skip firs element with column headers
                    if index != 0 and index < trade_days:
                        # append price to outdata in matching list (aligned by date)
                        outdata[index].append(float(splitter[index][4]))
                    # if end-of-data is reached...
                    elif index > trade_days:
                        # add 0s instead of prices
                        outdata[index].append(0)
            else:
                outdata[0].append(ticker)
                # iterate over list...
                for index, each in enumerate(splitdata):
...
ticker_list = ["BMV:AC", "BMV:FHIPO14"]
